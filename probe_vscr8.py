"""Probe 8: syndrome-balanced loss weighting to equalize per-syndrome gradients."""
import time, numpy as np, torch
import ssvr_qec as m

W0 = torch.ones(16, dtype=torch.float64)
W0[1:] = 25.0          # boost the 15 correction syndromes vs the no-error sector
...
for ep, lr, pr in stages:
    h = m.train_vscr(model, n_epochs=ep, batch=48, p_range=pr, noise='depolarizing',
                     lr=lr, lam=0.0, verbose=False, synd_w=W0)
    with torch.no_grad():
        R_all = m.recovery_unitary_batch(model())
    nov = sum(float(abs(torch.trace(m.C_SYNDS[s].conj().T @ R_all[s])) / m.DIM > 0.95)
              for s in range(16))
    print(f"stage {ep}@{lr}: val={h['fidelity'][-1]:.4f} locked={nov}/16 ({time.time()-t0:.0f}s)")
