"""Probe 19: multi-seed training + label-free syndrome diagnostic selection."""
...
def cond_fid(model, noise, p=0.07, M=150, seed=5):
    ...
    cf = Fs / np.maximum(Ws, 1e-12)
    return cf, Ws / M

for noise, stages in [('depolarizing',
                       [(600, 3e-2, (0.02, 0.08)), (400, 3e-2, (0.02, 0.15)), (1200, 1e-3, (0.02, 0.15))]),
                      ('amplitude_damping',
                       [(800, 3e-2, (0.01, 0.12)), (1200, 1e-3, (0.01, 0.12))])]:
    print(f"\n########## {noise} ##########")
    results = []
    for seed in [1234, 2024, 777, 31337]:
    ...
    best = results[0][3]
    print(f"  selected seed (best min_cf)")
    with torch.no_grad():
